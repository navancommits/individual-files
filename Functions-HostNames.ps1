...
function Initialize-HostNames {
    ...
    $cmSite=Check-HostNameExists "cm.$($HostDomain)"
    $cdSite=Check-HostNameExists "cd.$($HostDomain)"
    $idSite=Check-HostNameExists "id.$($HostDomain)"
    if ($cmSite -eq $false) {Add-HostsEntry "cm.$($HostDomain)"}
    if ($cdSite -eq $false) {Add-HostsEntry "cd.$($HostDomain)"}
    if ($idSite -eq $false) {Add-HostsEntry "id.$($HostDomain)"}
